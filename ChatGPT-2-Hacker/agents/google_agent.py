...
gpt_model = input(f"{YELLOW}[*] Please confirm the GPT model [gpt-3.5-turbo/gpt-4-turbo]\nEnter to default to gpt-4-turbo:{RESET}  ") or "gpt-4-turbo"
# default to gpt-4-turbo if no input
...
print(f"{YELLOW}>>> Building conversational memory for LLM agent...{RESET}")
...
# Put tools in toolkit - Langchain requires name & description
toolkit = [ 
    Tool(
        name = "Google Search",
        func=search.run,
