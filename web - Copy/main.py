...
def main(target):
    ...
    if re.match(r'^(\d{1,3}\.){3}\d{1,3}$', target):  # Check if target is an IP address
        print(f"{LIGHT_GREEN}Detected target as an IP address.{RESET}")
        ...
        if full_promptCheck:
            ...
            if user_confirmation("\nAre you ready to continue?"):
                for i in filtered_version_values:   # ['OpenSSH 8.4p1 Ubuntu 5ubuntu1.2', 'Apache 2.4.49']               
                    ...
                    if user_confirmation(f"{LIGHT_GREEN}[*] Advance with stage 2 LLM enrichment prompting?{RESET}"):
            ...
            for i in filtered_version_values:
                ...
                if user_confirmation(f"{LIGHT_GREEN}[*] Would you like to grab raw exploit code?"):
                    ...
                    if exploit_code.status_code == 200:
                        ...
                        if user_confirmation("Would you like a preview?"):
                            print(f"{YELLOW}==================================== CODE ====================================\n{RESET}")
                            print(data)
        ...
        if gobusterCheck:
            ...
            print(f"[*] Checking for suitability to run gobuster dir for subdomain enumeration and wpscan for wordpress sites ...")
            loading_bar(0.2)
        ...
        if wpbruteCheck:
            ...
            if user_confirmation("Run wpscan bruteforce against target?"):
...
if __name__ == "__main__":     
    ...
    parser.add_argument("--model", required=False, help="Select LLModels to use: gpt-3.5-turbo, gpt-4-turbo", default="gpt-4-turbo", type=str)
    group.add_argument("-vp" , "--vuln-prompt", required=False, help="Run only 1st LLM vulnerability discovery & enrichment on services found in nmap scan", action="store_true")
    group.add_argument("-fp", "--full-prompt", required=False, help="Run full 2-staged LLM vulnerability discovery & enrichment on services found in nmap scan", action="store_true")
    ...
    parser.add_argument("-s", "--sql-inject", required=False, help="Activates SQL Injection LLM Agent for autonomous web hacking", action="store_true")

    # Collate and append to output to specified file
    parser.add_argument("-oO", "--original-output", required=False, help="File to output pentest report results")

    # output to file and ask LLM to summarize
    parser.add_argument("-oX", "--sum-output", required=False, help="Enquires LLM to summarize pentest report results")
